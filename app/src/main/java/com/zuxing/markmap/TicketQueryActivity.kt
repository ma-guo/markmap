...
class TicketQueryActivity : AppCompatActivity() {
        ...
        lifecycleScope.launch {
            try {
                val response = withContext(Dispatchers.IO) {
                    ...
                    ticketService.queryTickets(date, fromStation, toStation).execute()
                }
    ...
    private fun parseResults(result: List<String>, stationMap: Map<String, String>): List<TicketInfo> {
        ...
        for (item in result) {
            ...
            val fromStation = stationMap[parts[6]] ?: parts[6]
            val toStation = stationMap[parts[7]] ?: parts[7]
            ...
            tickets.add(TicketInfo(trainNo, stationTrainCode, fromStation, toStation, startTime, arriveTime, duration, canBuy, seats, prices, date))
        }
